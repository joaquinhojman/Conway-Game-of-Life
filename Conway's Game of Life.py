...
def life_crear(mapa):
	...
	estado_del_juego=[]	
	for i in mapa:
		sublista_estado_del_juego =[]
		for j in i:
			
			if j ==".":
				sublista_estado_del_juego.append(False)
			elif j == "#":
				sublista_estado_del_juego.append(True)
		estado_del_juego.append(sublista_estado_del_juego)
...
def cant_adyacentes(life, f, c):
	...
	"""
	cant_celulas_adyacentes=0
	
	fila_arriba=(f-1)%len(life)
	fila_abajo=(f+1)%len(life)
	columna_izq=(c-1)%len(life[f])
	columna_der=(c+1)%len(life[f])

	if life[fila_arriba][c] == True:
		cant_celulas_adyacentes+=1
	if life[fila_abajo][c] == True:
		cant_celulas_adyacentes+=1	
	if life[f][columna_izq] == True:
		cant_celulas_adyacentes+=1	
	if life[f][columna_der] == True:
		cant_celulas_adyacentes+=1
	if life[fila_abajo][columna_der] == True:
		cant_celulas_adyacentes+=1
	if life[fila_abajo][columna_izq] == True:
		cant_celulas_adyacentes+=1	
	if life[fila_arriba][columna_der] == True:
		cant_celulas_adyacentes+=1
	if life[fila_arriba][columna_izq] == True:
		cant_celulas_adyacentes+=1	
			
	return cant_celulas_adyacentes
...
def celda_siguiente(life, f, c):
	...
	if celda== False and n ==3:	
		return True
	elif celda==True and (n==2 or n==3):
		return True
	else:
		return False
